...
{
    while (true) {
        cout << name << " is waiting for permit" << "\n";
        semaphore.Acquire();
        cout << name << " has got permit, new count is " << semaphore.NumTaken()
             << "\n";
        for (int i = 0; i < 3; ++i) {
            sleep_for(milliseconds(200));
            cout << name + " increments shared data: " << shared_data++ << "\n";
        }
        cout << name + " has released permit" << "\n";
        semaphore.Release();
    }
}
...
void DecrementThread(Semaphore& semaphore, string name)
{
    while (true) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            cout << name + " decrements shared data: " << shared_data-- << "\n";
        }
        cout << name + " has released permit" << "\n";
        semaphore.Release();
    }
...
int main(int argc, char* argv[])
{
    ...
    cout << "Semaphore with 1 permit has been created" << "\n";
