...
TreeWithSize FindKUnbalancedNodeHelper(const unique_ptr<Binary_tree_node<int>>&,
                                       int);
...
Binary_tree_node<int>* FindKUnbalancedNode(
...
int main(int argc, char* argv[])
{
    ...
    int k = 0;
    Binary_tree_node<int>* ans(FindKUnbalancedNode(root, k));
    assert(ans->data == 2);
    if (ans) {
        cout << ans->data << "\n";
    }
